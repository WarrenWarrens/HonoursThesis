...
// --- FIX: buffer for ICE candidates that arrive before setRemoteDescription ---
...
function startViewer(code) {
    ...
    ws.onmessage = async (event) => {
        ...
        if (msg.type === "error") {
            ...
            isMuted = true;
            setTimeout(() => { isMuted = false; }, msg.duration || 300000);
            return;
        }
        ...
        if (msg.type === "offer") {
            ...
            pc.ontrack = (event) => {
                remoteStream.addTrack(event.track);
                videoEl.srcObject = remoteStream;
                // Let the autoplay attribute handle playback; only call play() as a fallback
                videoEl.play().catch(err => console.warn("autoplay blocked:", err));
            };

            pc.onconnectionstatechange = () => {
                console.log("viewer connectionState:", pc.connectionState);
                // If the connection fails, show an error so the user knows to refresh
                if (pc.connectionState === "failed") {
                    errorEl.textContent = "Video connection failed — please refresh and try again.";
                }
            };
            pc.oniceconnectionstatechange = () => console.log("viewer iceConnectionState:", pc.iceConnectionState);
            ...
            try {
                ...
                // --- FIX: flush any candidates that arrived before setRemoteDescription finished ---
            ...
            if (!remoteDescriptionSet) {
                ...
                console.log("viewer: buffering ICE candidate (remote description not set yet)");
